...
app.put("/jokes/:id", (req, res) => {
  ...
  jokes[jokeIndex] = jokeReplace;
  res.json(jokeReplace);
});

//6. PATCH a joke
app.patch("/jokes/:id", (req, res) => {
  const jokeId = parseInt(req.params.id);
  const jokePrevious = jokes.find((joke) => joke.id === jokeId);
  const jokePatch = {
    id: jokeId,
    jokeText: req.body.jokeText || jokePrevious.jokeText,
    jokeType: req.body.jokeType || jokePrevious.jokeType,
  };

  const jokeIndex = jokes.findIndex((joke) => joke.id === jokeId);
  jokes[jokeIndex] = jokePatch;
  res.json(jokePatch);
});
...
app.delete("/jokes/:id", (req, res) => {
  const jokeId = parseInt(req.params.id);
  const specificJoke = jokes.findIndex((joke) => joke.id === jokeId);
  if (specificJoke > -1) {
